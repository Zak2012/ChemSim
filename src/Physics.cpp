...
bool fx_Collide(Rect2D A, Ray2D B)
{
    return fx_Collide(B,A);
}

bool fx_Collide(Rect2D A, Ray2D B, glm::vec2 &C)
{
    return fx_Collide(B,A,C);
}


bool fx_Collide(Circle3D A, Line3D B)
{
    if (fx_Collide(B.Start, A) || fx_Collide(B.End, A))
    {
        return true;
    ...
    float dot = glm::dot((B.End - B.Start), A.Pos);
    glm::vec3 closest = B.Start + (dot * (B.End - B.Start));
    if (fx_Collide(closest, B))
    {
        return true;
    }
...
bool fx_Collide(Line3D A, Rect3D B, glm::vec3 &C)
{
    Ray3D Ray;
    Ray.Pos = A.Start;
    Ray.Dir = glm::normalize(A.End - A.Start);

    if (!fx_Collide(Ray, B, C))
    {
        return false;
    }

    return glm::distance2(Ray.Pos, C) <= glm::distance2(A.Start, A.End);
}
